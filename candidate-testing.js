...
function gradeQuiz(candidateAnswers) {
  ...
  let candidateAnswersLower = candidateAnswers.map((item) => {
    return item.toLowerCase();
  });
  
  let correctAnswersLower = correctAnswers.map((item) => {
    return item.toLowerCase();
  });
  let rightAnswer=0;
 
    
 for(let i=0; i<candidateAnswersLower.length; i++){
 ...
 let grade=(rightAnswer/5 *100);  

console.log (`Congratulations, ${candidateName}, You got a score of ${grade} %!`)
  return grade;
}
